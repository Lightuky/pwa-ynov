  ...
  <div class="d-flex flex-column flex-center align-items-center h-100">
    ...
    <template v-if="isLoggedIn">
      ...
      <template v-if="taskList && taskList !== 'null' && task && task !== 'null'">
        <div class="mainTitle mb-3">Supprimer la tâche</div>
        <q-card class="col-4 formBlock px-3 py-4">
          <q-card-section>
            <div class="deleteDisclaimer py-4 text-black">Vous êtes sur le point de supprimer votre tâche êtes vous sûr de vouloir faire ça ?</div>
...
export default defineComponent({
  ...
  methods: {
    ...
    onSubmit () {
      const self = this
      axios.delete('http://127.0.0.1:8000/api/tasks/' + this.task.id, { headers: { 'api-key': this.session.api_token } })
        .then(function () {
