...
<template>
  <div class="d-flex flex-column flex-center align-items-center h-100">
    ...
    <template v-if="isLoggedIn">
      ...
      <template v-if="taskList && taskList !== 'null'">
        <div class="mainTitle mb-3">Créer une tâche</div>
        <q-card class="col-4 formBlock px-3 py-4">
          <q-card-section>
            <q-form class="q-gutter-md" @submit="onSubmit" autocorrect="off" autocapitalize="off" autocomplete="off" spellcheck="false">
              ...
              <q-input outlined v-model="form.name" label="Ex: Acheter des pommes" class="q-mt-sm q-mb-md q-gutter-none ms-2 pb-1" type="text"
                       :rules="[val => val && val.length > 0 || 'Veuillez rentrer un nom']" lazy-rules/>
              <div class="inputLabel ms-2 mt-0">Description</div>
              <q-input outlined v-model="form.description" label="Décrivez votre tâche" class="q-mt-sm q-mb-md q-gutter-none ms-2 pb-1" type="textarea"
    ...
    <template v-else>
      <div class="mainTitleAlt mt-5">Attention 🤚</div>
      <div class="subLabel text-black my-4">Veuillez vous connecter pour pouvoir accéder à l'ajout</div>
...
export default defineComponent({
  ...
  methods: {
    getTasksList: async function () {
      ...
      this.form.taskslist_id = this.taskListId
    },
    onSubmit () {
      const self = this
      axios.post('http://127.0.0.1:8000/api/tasks/', this.form, { headers: { 'api-key': this.session.api_token } })
        .then(function () {
